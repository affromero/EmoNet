...
if __name__ == '__main__':    
	...
	parser.add_argument('--metapath', type=int, default=0)
	parser.add_argument('--aligned', action='store_true', default=False)
	args = parser.parse_args()
	os.environ['CUDA_VISIBLE_DEVICES']=args.gpu

	datasets = ['/home/afromero/datos/Databases/EmotionNet', '../data/EmotionNet2']

	folder_root = datasets[args.metapath]#'/home/afromero/datos/Databases/EmotionNet'
	txt_root = os.path.join(folder_root,'txt')
	org_root = os.path.join(folder_root,'Images')
	...
	if args.aligned: 
		...
		f = open(_f, 'w')
		for rs_file in face_files: f.writelines(rs_file+'\n')
		f.close()
		# ipdb.set_trace()
	...
	for org_file in tqdm.tqdm(org_files,	desc='Resizing Faces to %d'%(args.img_size)):
		rs_file = org_file.replace('Images', 'Faces')
		...
		if args.aligned:
			face_aligned_resized = resized_file.replace('Faces', 'Faces_aligned')
			if not os.path.isfile(face_aligned_resized): 
				get_resize(rs_file.replace('Faces', 'Faces_aligned'), face_aligned_resized, args.img_size)
