...
def get_data(aligned=False):
	...
	for txt in txt_files:
		lines.extend([line.strip().replace('Sequences', replace) for line in sorted(open(txt).readlines())])
	lines.sort()
	return lines
...
def imshow_data(line):
	...
	aus = [cfg.AUs_name_es[i] for i in range(len(cfg.AUs_name_es)) if labels[i]==1]
	# print(str(labels))
